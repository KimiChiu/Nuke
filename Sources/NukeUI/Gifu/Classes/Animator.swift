...
class Animator {
    ...
    // if the animation was stopped manually, to prevent auto-start when the view is not visible.
    var manuallyStopped: Bool = true;
    // if the animation was stopped manually, to show the first frame, we have to wait for the first display call.
    var firstFrameWait: TimeInterval = 0.5;
    ...
    func animate(withGIFNamed imageName: String, size: CGSize, contentMode: UIView.ContentMode, loopCount: Int = 0, preparationBlock: (() -> Void)? = nil, animationBlock: (() -> Void)? = nil) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + firstFrameWait) { [weak self] in
            guard let manuallyStopped = self?.manuallyStopped, manuallyStopped else {
                return
            }
            self?.stopAnimating()
        }
